    ...
    <div class="card">
        ...
        <form action="edit_action.php" method="POST" autocomplete="off">
            ...
            <input type="text" name="phone" value="<?= htmlspecialchars($contact['phone']) ?>"
                   required pattern="[0-9]+" title="Nomor telepon hanya boleh angka">

            <label>Asal (opsional)</label>
            <input type="text" name="asal" value="<?= htmlspecialchars($contact['asal']) ?>">

            <label>Catatan (opsional)</label>
            <textarea name="catatan"><?= htmlspecialchars($contact['catatan']) ?></textarea>
